    ...
    with open(file_path, "r", encoding="utf-8") as f:
        if token_encoding:
            ...
            chars_per_token = estimate_chars_per_token(
                buffer, tiktoken.get_encoding(token_encoding)
            )

            for line in f:
                buffer += line
                while len(buffer) / chars_per_token > chunk_size + delta:
                    split_point = find_best_boundary(
                        buffer,
                        chunk_size,
            ...
            for line in f:
                ...
                while len(buffer) > chunk_size + delta:
                    left_of, right_of = find_two_closest_occurrences(
                        buffer, chunk_size, delta=delta, key=key
                    )
...
def rough_chunk_text(
    ...
    if token_encoding:
        chars_per_token = estimate_chars_per_token(
            text, tiktoken.get_encoding(token_encoding)
        )
        while len(text) / chars_per_token >= chunk_size:
            split_point = find_best_boundary(
                text, chunk_size, tiktoken.get_encoding(token_encoding), delta, key
            )
        ...
        while len(text) >= chunk_size:
            left_of, right_of = find_two_closest_occurrences(
                text, chunk_size, delta=delta, key=key
            )
            split_point = get_best_split_point(left_of, right_of, chunk_size)
            chunks.append(text[:split_point])
            text = text[split_point - overlap :]
...
def kamradt_chunk(
    text: str, token_encoding: str, chunk_size: int = 200, threshold: float = 0.95
) -> List[str]:
    """
    ...
    with open(file_path, "r", encoding="utf-8") as f:
        for line in f:
            ...
            if len(buffer) >= parent_chunk_size:
                left_of, right_of = find_two_closest_occurrences(
                    buffer, parent_chunk_size, delta
                )
                split_point = get_best_split_point(left_of, right_of, parent_chunk_size)
                parent_chunk = buffer[:split_point]
                buffer = buffer[split_point - overlap :]
                chunks = strict_chunk_text(
                    parent_chunk, chunk_size=chunk_size, overlap=overlap
                )
    return chunks
...
def find_two_closest_occurrences(
    text: str, index: int, delta: int, key: List[str] = [".", "?", "!"]
...
def get_best_split_point(left_of: int | None, right_of: int | None, index: int) -> int:
    """
...
def estimate_chars_per_token(
    text: str, tokenizer: tiktoken.Encoding, sample_size: int = 1000
) -> float:
    """
...
def find_best_boundary(
    ...
    while search_left or search_right:
        if search_right:
            ...
            while (
                text[right_pos] not in key
                and right_pos < start_idx + search_delta
                and right_pos < len(text) - 1
            ):
                right_pos += 1
        ...
        if search_left:
            ...
            while (
                text[left_pos] not in key
                and start_idx - left_pos < search_delta
                and left_pos > 0
            ):
                left_pos -= 1
    ...
    return (
        left_pos
        if abs(left_token_count - chunk_size) <= abs(right_token_count - chunk_size)
        else right_pos
    )
