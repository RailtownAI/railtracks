...
from railtracks.events._base import UNSET,  LLMSpatialParent, NoSpatialParent, NodeSpatialParent, SpatialParent, SessionEventBase
...
def _get_llm_parents(event: LLMMessageBase) -> tuple[NodeSpatialParent, LLMParent]:
    ...
    assert llm_call_details is not None, ("LLM call ID should be set in the context when publishing an LLM event.")
    assert node_id is not None, ("Node ID should be set in the context when publishing an LLM event.")

    return NodeSpatialParent(node_id), LLMParent(llm_invoke_id=llm_call_details.call_id, llm_model_id=llm_call_details.type_id)
