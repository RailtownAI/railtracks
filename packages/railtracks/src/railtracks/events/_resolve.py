...
from railtracks.events._base import LLMParent, NodeParent, NoParent, Parent
...
def _parent_from_anchor(entry: ScopeEntry, middleware_id: str | None) -> Parent:
    """Build the `Parent` for a matched anchor entry (+ its intervening middleware)."""
    if entry.kind in _NODE_KINDS:
        return NodeParent(node_id=entry.id, middleware_id=middleware_id)
    if entry.kind is ScopeKind.LLM:
        return LLMParent(llm_id=entry.id, middleware_id=middleware_id)
...
def node_parent(scope: ScopeLink[ScopeEntry] | None) -> Parent:
    """Parent of a running node event (NodeInvocation/Response/Failure/Destruction)."""
    return resolve_parent(scope, self_kind=ScopeKind.NODE, anchor_kinds=_NODE_KINDS)


def node_creation_parent(scope: ScopeLink[ScopeEntry] | None) -> Parent:
    """Parent of a `NodeCreation` event (self not yet on the chain)."""
    return resolve_parent_at_creation(scope, anchor_kinds=_NODE_KINDS)


def llm_parent(scope: ScopeLink[ScopeEntry] | None) -> Parent:
    """Parent of an LLM event — the enclosing node (+ intervening middleware)."""
    return resolve_parent(scope, self_kind=ScopeKind.LLM, anchor_kinds=_NODE_KINDS)


def middleware_parent(scope: ScopeLink[ScopeEntry] | None) -> Parent:
    """Parent of a general (non-model) middleware event — the node it runs on."""
    return resolve_parent(
        scope, self_kind=ScopeKind.MIDDLEWARE, anchor_kinds=_NODE_KINDS
    )


def model_middleware_parent(scope: ScopeLink[ScopeEntry] | None) -> Parent:
    """Parent of a model-middleware event — the LLM call it wraps (anchors on the LLM
    entry, not the node)."""
    return resolve_parent(
        scope, self_kind=ScopeKind.MIDDLEWARE, anchor_kinds=frozenset({ScopeKind.LLM})
    )
