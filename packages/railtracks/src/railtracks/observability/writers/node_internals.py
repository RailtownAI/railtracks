...
class NodeInternalsCollector:
    ...
    Satisfies the :class:`~railtracks.observability.writers.base.Writer` protocol so
    it can be registered on the ``Observer`` like any other sink. ``write`` only
    buffers; the fold happens in :meth:`internals_for`, which keeps it independent of
    event ordering — guardrail events, for instance, arrive before the LLM event that
    identifies which node they belong to.

    State is keyed by session so a process running more than one session keeps them
    apart, and it deliberately survives ``shutdown()``: the observer is torn down
    before ``Session.__exit__`` serializes the document.
    """
    ...
    async def shutdown(self) -> None:
        # Intentionally not clearing: the session reads its slice after teardown.
        pass
...
def _request_details(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        # a failure carries no response, which the legacy trace recorded as a null
        # output alongside null token/cost fields
