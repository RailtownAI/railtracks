...
# Filter out Cohere and HuggingFace LLMs for these tests
# Cohere: Filtered due to API incompatibility issues
# HuggingFace: Filtered due to API key permission issues with Inference Providers
llm_map_filtered = {
    k: v
    for k, v in llm_map.items()
    if "cohere" not in k.lower() and "huggingface" not in k.lower()
}
...
test_cases = [
    {
        ...
        "validator": lambda response: (
            response.content.name == "Railtown" and response.content.age == 6
        ),
    ...
    },
    {
        ...
        "validator": lambda response: (
            response.content.name == "Alice"
            and response.content.age == 25
            and response.content.address.city == "Vancouver"
            and response.content.address.country == "Canada"
        ),
        ...
        if llm.stream:
            for chunk in response:
                assert isinstance(chunk, (str, StringResponse)), (
                    "The response should be either string or the final response"
                )
