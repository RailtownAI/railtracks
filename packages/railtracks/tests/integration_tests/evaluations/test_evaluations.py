...
from railtracks.evaluations.point import AgentDataPoint, data_points_from_payload
...
def test_correct_llm_values_extracted_from_session(session_payload):
    """Values written into the session JSON reach the AgentDataPoint unchanged."""
    adps = data_points_from_payload(session_payload)
...
def test_correct_tool_values_extracted_from_session(session_payload):
...
def test_agent_io_extracted_from_session(session_payload):
...
def test_session_and_agent_ids_preserved(session_payload):
...
def test_multiple_payloads_yield_one_adp_each(two_session_payloads):
    """Each session payload yields one AgentDataPoint."""
    adps = [dp for payload in two_session_payloads for dp in data_points_from_payload(payload)]
    assert len(adps) == 2
...
def test_llm_evaluator_input_token_values_match_session(two_session_payloads):
...
def test_llm_evaluator_latency_values_match_session(two_session_payloads):
...
def test_llm_evaluator_aggregate_mean_reflects_session_values(two_session_payloads):
...
def test_llm_evaluator_agent_data_ids_match_extracted_adps(two_session_payloads):
...
def test_tool_evaluator_failure_rate_zero_for_healthy_runs(two_session_payloads):
...
def test_tool_evaluator_failure_rate_propagates_from_failed_session():
    ...
    adps = [dp for payload in payloads for dp in data_points_from_payload(payload)]
    result = ToolUseEvaluator().run(adps)
    failure_rates = [r for r in result.metric_results if r.result_name.startswith("FailureRate")]
    assert 1.0 in [r.value for r in failure_rates]


def test_tool_evaluator_runtime_values_match_session(two_session_payloads):
...
def test_evaluate_with_real_evaluators_returns_evaluation_results(two_session_payloads):
...
def test_evaluate_evaluator_results_wired_into_evaluation_result(two_session_payloads):
...
def test_evaluate_metrics_map_contains_all_metrics(two_session_payloads):
...
def test_evaluate_agent_node_ids_reference_extracted_adps(two_session_payloads):
...
def test_evaluate_multi_agent_filter_via_agents_param():
...
def test_payload_from_real_result_is_json_serializable(two_session_payloads):
...
def test_payload_callback_receives_real_result_dict(two_session_payloads):
