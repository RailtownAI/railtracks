...
class TestToolCall:
    ...
    def test_tool_response_str(self):
        tool_response = ToolResponse(
            identifier="123", name="example_tool", result="success"
        )
    ...
    def test_invalid_tool_call(
        self, invalid_identifier, invalid_name, invalid_arguments, expected_exception
    ):
...
class TestToolResponse:
    ...
    def test_invalid_tool_response(
        self, invalid_identifier, invalid_name, invalid_result, expected_exception
    ):
        with pytest.raises(expected_exception):
            ToolResponse(
                identifier=invalid_identifier, name=invalid_name, result=invalid_result
            )
