...
@pytest.fixture
def patch_node():
    patcher = patch("railtracks.builtnodes._node_builder.Node", DummyNode)
    patcher.start()
    yield DummyNode
    patcher.stop()
...
@pytest.fixture
def patch_llm_base():
    patcher = patch("railtracks.builtnodes._node_builder.LLMBase", DummyLLMBase)
    patcher.start()
    yield DummyLLMBase
    patcher.stop()
...
def patch_outputless_toolcall_llm():
    patcher = patch("railtracks.builtnodes._node_builder.OutputLessToolCallLLM", DummyOutputLessToolCallLLM)
    patcher.start()
...
def patch_function_node():
    patcher = patch("railtracks.builtnodes._node_builder.DynamicFunctionNode", DummyFunctionNode)
    patcher.start()
