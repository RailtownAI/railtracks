...
class TestConversion:
    ...
    async def test_metadata_source_is_popped_into_document_source(self):
        loader = _AsyncLoader([
            FakeLCDocument(page_content="x", metadata={"source": "s3://bucket/key", "page": 1})
        ])
        docs = await LangChainLoaderAdapter(loader).aload()
    ...
    async def test_explicit_source_overrides_metadata_source(self):
        loader = _AsyncLoader([
            FakeLCDocument(page_content="x", metadata={"source": "ignored", "page": 1})
        ])
    ...
    async def test_document_type_applied_to_every_document(self):
        ...
        docs = await LangChainLoaderAdapter(loader, document_type=DocumentType.MARKDOWN).aload()
...
class TestStreamingPriority:
    ...
    async def test_lazy_load_used_when_no_alazy_load(self):
        ...
        loader = _LazyOnlyLoader([FakeLCDocument(page_content=f"d{i}") for i in range(3)])
        docs = await LangChainLoaderAdapter(loader).aload()
    ...
    async def test_lazy_load_is_streamed_one_at_a_time(self):
        ...
        loader = _LazyOnlyLoader([FakeLCDocument(page_content=f"d{i}") for i in range(5)])
...
class TestSyncWrapper:
    def test_load_returns_same_documents_as_aload(self):
        ...
        assert [d.content for d in sync_docs] == [d.content for d in async_docs] == ["a", "b"]
