...
import pytest
from railtracks.execution.coordinator import Coordinator, CoordinatorState, Job
...
def all_execution_modes(mock_execution_strategy):
    ...
    return dict.fromkeys(get_args(ExecutionConfigurations), mock_execution_strategy)
...
@pytest.mark.asyncio
async def test_coordinator_submit_adds_job_and_executes(
    coordinator, mock_task, mock_execution_strategy
):
    result = await coordinator.submit(
        mock_task, list(coordinator.execution_strategy.keys())[0]
    )
...
def test_coordinator_handle_item_success_and_failure(coordinator, mock_task):
    ...
    msg_fail = RequestFailure(
        request_id="req-2", node_state="dummy", error=Exception("fail")
    )
...
def test_coordinator_shutdown_calls_all_strategies(
    coordinator, mock_execution_strategy
):
