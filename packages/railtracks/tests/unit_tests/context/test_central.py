...
def test_is_context_present_and_active(monkeypatch, make_runner_context_vars):
    ...
    monkeypatch.setattr(
        central, "runner_context", mock.Mock(get=mock.Mock(return_value=rt))
    )
...
def test_get_publisher_returns_publisher(
    monkeypatch, make_session_context_mock, make_runner_context_vars
):
    pub = mock.Mock()
    rt = make_runner_context_vars(
        session_context=make_session_context_mock(publisher=pub)
    )
    monkeypatch.setattr(
        central, "runner_context", mock.Mock(get=mock.Mock(return_value=rt))
    )
...
@pytest.mark.asyncio
async def test_activate_publisher(
    monkeypatch, make_runner_context_vars, make_session_context_mock
):
    pub = mock.AsyncMock()
...
@pytest.mark.asyncio
async def test_shutdown_publisher(
    monkeypatch, make_runner_context_vars, make_session_context_mock
):
    pub = mock.AsyncMock()
...
def test_get_parent_id(
    ...
    rt = make_runner_context_vars(
        session_context=make_session_context_mock(current_node_id="parent-abc")
    )
    monkeypatch.setattr(
        central, "runner_context", mock.Mock(get=mock.Mock(return_value=rt))
    )
...
def test_get_and_set_global_config(monkeypatch):
    ...
    monkeypatch.setattr(
        central,
        "global_executor_config",
        mock.Mock(get=mock.Mock(return_value=config), set=mock.Mock()),
    )
...
def test_get_and_set_local_config(
    monkeypatch, make_runner_context_vars, make_session_context_mock
):
    config = mock.Mock()
    rt = make_runner_context_vars(
        session_context=make_session_context_mock(executor_config=config)
    )
    monkeypatch.setattr(central, "safe_get_runner_context", mock.Mock(return_value=rt))
...
def test_middleware_fired_node_lands_under_middleware():
    ...
    with manager.enter_node("node-1"):
        with manager.enter_middleware("guard") as middleware_id:
            ...
            assert scope.value == ScopeEntry(
                ScopeKind.MIDDLEWARE, middleware_id, type_id="guard"
            )
            with manager.enter_node("node-2"):
                ...
                assert (
                    central.get_current_scope().parent.value.kind
                    is ScopeKind.MIDDLEWARE
                )
