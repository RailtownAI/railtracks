...
from railtracks.observability_bridge import make_event
...
def test_returns_event_inside_session():
    event = _run_in_session(
        "session-abc",
        lambda: make_event("node.create", {"node_id": "n1"}),
    )
...
def test_payload_stored_as_is():
    ...
    event = _run_in_session("s1", lambda: make_event("test.event", payload))
    assert event.payload == payload


def test_stamp_is_tz_aware_utc():
    before = datetime.now(timezone.utc)
    event = _run_in_session("s1", lambda: make_event("x", {}))
...
def test_event_type_is_free_form():
    event = _run_in_session(
        "s1", lambda: make_event("totally.made.up.event.name", {})
    )
...
def test_scope_id_propagates_across_copy_context_boundary():
    ...
    catches it — make_event would raise ContextError, or return the wrong scope.
    """
    captured: list[Event] = []
    ...
    def _outer():
        ...
        async def _inner():
            return make_event("test.event", {"marker": "in-inner"})
...
def test_leaves_runner_context_none_outside_helper():
    ...
    _run_in_session("leaky-session", lambda: make_event("x", {}))
