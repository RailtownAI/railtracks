...
def test_create_mcp_server_new_server_registers_tools(
    ...
    mock_fast_mcp.return_value = mcp_instance
    mock_node_cls.tool_info.return_value = mock_node_info
    # Patch MCPTool and func_metadata for full test
    mock_mcp_tool.return_value = "mcp-tool"  # so value is easy to check
    ...
    mock_fast_mcp.assert_called_with("Srv")
    assert tool_manager._tools[mock_node_info.name] == "mcp-tool"
    tool_args = mock_mcp_tool.call_args[1]
...
def test_create_mcp_rasies_error_if_wrong_type(mock_node_cls, mock_executor_config):
...
def test_create_mcp_server_raises_if_wrong_type(mock_node_cls, mock_executor_config):
    with pytest.raises(ValueError, match="must be an instance of FastMCP"):
        create_mcp_server([mock_node_cls], fastmcp="notafastmcp")
