...
import pytest
from railtracks.state.forest import AbstractLinkedObject, Forest
...
def mock_linked_object():
    def _mock_linked_object(identifier, message, stamp, parent):
        return MockLinkedObject(identifier, stamp, parent, message)

    return _mock_linked_object
    ...
    class DummyStamper:
        class DummyStamp:
            ...
            def __eq__(self, other):
                return isinstance(other, DummyStamper.DummyStamp)

        def __init__(self):
            self.stamps = []

        def create_stamp(self, msg):
            self.stamps.append(msg)
            return self.DummyStamp(msg)

        def stamp_creator(self):
            return lambda msg="": self.create_stamp(msg)
...
def patch_rt_logger(monkeypatch):
    ...
    monkeypatch.setattr("railtracks.state.state.get_rt_logger", lambda: MagicMock())
