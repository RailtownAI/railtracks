...
def test_linkednode_to_vertex_includes_hierarchy(
    dummy_node_factory, linked_node_factory
):
    base_id = str(uuid.uuid4())
    parent_stamp = Stamp(1, 0, "init")
    parent_ln = linked_node_factory(
        base_id, parent_stamp, parent=None, pretty="parent", details={"a": 1}
    )
    child_stamp = Stamp(1, 1, "child")
    child_ln = linked_node_factory(
        base_id, child_stamp, parent=parent_ln, pretty="child", details={"b": 2}
    )
