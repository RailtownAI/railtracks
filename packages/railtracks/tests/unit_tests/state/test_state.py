...
def test_rcstate_init_and_is_empty(
...
def test_rcstate_shutdown_invokes_coordinator(
    dummy_execution_info, dummy_executor_config, mock_coordinator, mock_publisher
):
    state = RTState(
        dummy_execution_info, dummy_executor_config, mock_coordinator, mock_publisher
...
def test_create_node_and_request(
    dummy_execution_info, dummy_executor_config, mock_coordinator, mock_publisher
):
    state = RTState(
        dummy_execution_info, dummy_executor_config, mock_coordinator, mock_publisher
...
@pytest.mark.asyncio
async def test_cancel_updates_request(
...
@pytest.mark.asyncio
async def test_cancel_asserts_if_node_id_is_missing(
    dummy_execution_info,
    dummy_executor_config,
    mock_coordinator,
    mock_publisher,
...
def test_info_and_get_info_filters_and_returns(
    dummy_execution_info,
    dummy_executor_config,
    mock_coordinator,
    mock_publisher,
...
@pytest.mark.asyncio
async def test__handle_failed_request_fatal_and_config(
    dummy_execution_info, dummy_executor_config, mock_coordinator, mock_publisher
):
    state = RTState(
        dummy_execution_info, dummy_executor_config, mock_coordinator, mock_publisher
    ...
    output = await state._handle_failed_request("name", "rid", FatalNodeError())
    assert isinstance(output, type(Failure(exc)))


@pytest.mark.asyncio
async def test__handle_failed_request_default(
    dummy_execution_info, dummy_executor_config, mock_coordinator, mock_publisher
):
    state = RTState(
        dummy_execution_info, dummy_executor_config, mock_coordinator, mock_publisher
...
def test_publisher_subscribe_called_on_init(
    dummy_execution_info, dummy_executor_config, mock_coordinator, mock_publisher
):
    RTState(
        dummy_execution_info, dummy_executor_config, mock_coordinator, mock_publisher
...
def test_create_new_request_set_asserts_on_missing_node(
    dummy_execution_info, dummy_executor_config, mock_coordinator, mock_publisher
):
    state = RTState(
        dummy_execution_info, dummy_executor_config, mock_coordinator, mock_publisher
