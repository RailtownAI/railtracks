...
def test_open_tails_deep_structure(req_template_factory):
    ...
    l_ = req_template_factory(source_id="K", sink_id="L", output="closed", step=3)
    all_reqs = [j, k, l_]
...
def test_forest_children_and_children_requests_complete(
    req_forest, req_template_factory
):
    a = req_template_factory(
        identifier="A", source_id=None, sink_id="A", output="ins", step=1
    )
    b = req_template_factory(
        identifier="B", source_id="A", sink_id="B", output="ok", step=2
    )
    c = req_template_factory(
        identifier="C", source_id="A", sink_id="C", output="ok", step=3
    )
    ...
    assert {x.sink_id for x in children} == {"B", "C"}
...
def test_forest_get_request_from_child_id(req_forest, req_template_factory):
    ...
    b = req_template_factory(
        identifier="B", source_id="A", sink_id="B", output=None, step=2
    )
    for req in (a, b):
...
def test_forest_open_tails_and_insertion_request(req_forest, req_template_factory):
    a = req_template_factory(
        identifier="A", source_id=None, sink_id="A", output=None, step=1
    )
    b = req_template_factory(
        identifier="B", source_id="A", sink_id="B", output="out", step=2
    )
    req_forest._update_heap(a)
    req_forest._update_heap(b)
    ...
    assert req_forest.answer is None  # answer property retrieves insertion output
...
def test_forest_to_edges_returns_all(req_forest, req_template_factory):
    a = req_template_factory(
        identifier="A", source_id=None, sink_id="A", output="foo", step=1
    )
    b = req_template_factory(
        identifier="B", source_id="A", sink_id="B", step=2, output="bar"
    )
    req_forest._update_heap(a)
    req_forest._update_heap(b)
