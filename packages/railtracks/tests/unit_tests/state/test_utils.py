...
def test_extract_single_parent_branch(req_template_factory, linked_node_factory):
    # Tree: root->A, A->B, A->C
    r_root = req_template_factory(
        identifier="r0", source_id=None, sink_id="A", step=1, output="root"
    )
...
def test_multiple_parents_duplicate_id_assertion(
    req_template_factory, linked_node_factory
):
...
def test_multiple_nonoverlapping_parents(req_template_factory, linked_node_factory):
    ...
    nodes = {
        "A": linked_node_factory("A", ra.stamp),
        "B": linked_node_factory("B", rb.stamp),
    }
