...
def test_empty_starter():
    ...
    with (
        patch(
            "railtracks.state.info.RequestForest",
        ...
        patch(
            "railtracks.state.info.NodeForest",
            return_value=MagicMock(heap=lambda: [], to_vertices=lambda: []),
        ),
        patch(
            "railtracks.state.info.StampManager",
            return_value=MagicMock(_step=0, all_stamps=[]),
        ),
    ):
...
def test_get_info_with_string_id_valid(monkeypatch, empty_info):
    ...
    with patch(
        "railtracks.state.info.create_sub_state_info",
        return_value=(mock_new_nf, mock_new_rf),
    ):
