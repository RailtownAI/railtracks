...
To author a rail, subclass `InputGuard` or `OutputGuard` and implement `__call__(self, event) -> GuardrailDecision`; see [Custom Guards](overview.md#custom-guards) for the pattern (and the decorator API for quick, local guards). To make it a shipped add-on:
...
We would love your contribution. See also the general [prebuilt add-on contribution guide](../prebuilt/contributions.md).
