...
| Decorator | Scope | Runs | Function signature |
|---|---|---|---|
| `rt.wrap_node` | Node | Wraps the whole node call — you decide if/how many times the inner call runs | `async fn(call, *args, **kwargs) -> result` |
| `rt.after_node` | Node | Once, after the node completes successfully (skipped if it raises) | `fn(result) -> result` (sync or async) |
| `rt.wrap_llm` | Model | Wraps the whole model call — you decide if/how many times the inner call runs | `async fn(llm_call, message_history, schema, tools) -> response` |
| `rt.before_llm` | Model | Once, before each model call, to transform the inputs | `fn(message_history, schema, tools) -> (message_history, schema, tools)` (sync or async) |
| `rt.after_llm` | Model | Once, after each successful model call | `fn(response) -> response` (sync or async) |
