...
Railtracks ships a suite of prebuilt middleware so you can add common behavior to an agent without writing it yourself. Every add-on below is a normal middleware; you attach it exactly like your own, on the `middleware=` (node) or `model_middleware=` (model) slot. For the difference between the two slots, see [Middleware Overview](../overview.md); to build your own, see [Custom Middleware](../custom.md).

The **Slot** column tells you where each add-on can be attached:
...
!!! Note
    Don't see what you are looking for? [Create your own middleware](../custom.md) for a custom solution, or [contribute a prebuilt add-on](contributions.md) so others can reuse it.
